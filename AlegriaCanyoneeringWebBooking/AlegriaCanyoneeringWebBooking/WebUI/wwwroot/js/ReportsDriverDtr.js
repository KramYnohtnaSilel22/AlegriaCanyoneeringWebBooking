$(document).ready(function () {
    ...
    $("#btnDownloadExcel").on("click", async function () {
        ...
        ws.columns = [
            ...
            { width: 16 },  // Ref ID
            { width: 16 },  // No. of Trips
            { width: 18 }   // Total Passengers
        ];

        // ---- Government header ----
        const setCell = (addr, value, bold, size, align) => {
        ...
        ws.mergeCells("A5:E5"); setCell("A5", "Driver Daily Time Record", true, 13);
        ws.mergeCells("A6:E6"); setCell("A6", `${dateFromText} - ${dateToText}`, true, 10);
        ws.getRow(7).height = 5;

        // ---- Table header row 8 ----
        const headers = ["Seq.", "Driver Name", "Ref ID", "No. of Trips", "Total Passengers"];
        ...
        if (tbody) {
            Array.from(tbody.querySelectorAll("tr")).forEach(tr => {
                ...
                if (cells.length >= 5 && !cells[1].includes("No DTR")) {
                    const row = ws.addRow([
                        Number(cells[0]) || 0,  // Seq
                        cells[1],               // Driver Name
                        cells[2],               // Ref ID
                        Number(cells[3]) || 0,  // No. of Trips
                        Number(cells[4]) || 0   // Total Passengers
                    ]);
        ...
        if (tfoot) {
            ...
            const totalRow = ws.addRow([
                "", "", "TOTAL:",
                Number(tf[tf.length - 2].replace(/[^0-9]/g, "")) || 0,
        ...
        ws.getCell(`A${noteIdx}`).value = "System Generated Report - Driver DTR Summary";
